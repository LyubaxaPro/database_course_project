...
irqreturn_t irq_handler(int irq, void *dev)
{
  ...
  if(irq == KEYB_IRQ)
  {
    ...
    printk(KERN_INFO "=== Interrupt - KEYBOARD! ===\n");
...
static int __init my_module_init(void)
{
  ...
  //Драйверы регистрируют обработчик аппаратного прерывания и разрешают определенную линию irq посредством функции request_irq:
  // KEYB_IRQ номер irq
  // указ-ль на наш обработчик прерывания
  // IRQF_SHARED флаг разделение(совместное использование) линии IRQ с другими устройствами
  // "keyboard" (ASCII текст, представляющий устройство, связанное с прерыванием)
